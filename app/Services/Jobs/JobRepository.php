...
class JobRepository {
	...
	public function getBySearch($search,$page=0)
	{
		...
		$result = $models->orderBy('created_at', 'DESC')->offset($page)->limit(10)->get();
	...
	public function buildAttr($model, $build_connote = false)
	{
		$updated_label = '';
		if($model->updated_at!=''){
			$updated_label = helperThaiFormat($model->updated_at, true).' เวลา '.helperDateFormat($model->updated_at, 'H:i น.');
		}else{
			$updated_label = helperThaiFormat($model->created_at, true).' เวลา '.helperDateFormat($model->created_at, 'H:i น.');
		}
		...
		$model->updated_label = $updated_label;
		...
		if($model->photo or $model->photo){
			if($model->photo){
				$photo = 'http://af1express.com/api/ajax/uploads/sign/'.$model->id.'_sign.png';//urlContent().'/'.$model->photo;
			}
		}else{
		...
		$model->notes = !empty($model->notes) ? $model->notes : '';
