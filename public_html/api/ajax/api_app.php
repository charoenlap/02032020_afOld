<?php 
	...
	if(isset($_GET['type'])){
		if($_GET['type']=='update_name_cus'){
			$result = array();
			$data_now = date('Y-m-d H:i:s');
			$sql = "UPDATE jobs SET 
			notes = '".$_GET['val']."',
			receiver_name = '".$_GET['val']."',
			updated_at='".$data_now."', 
			`status` = 'complete'
			WHERE id='".$_GET['job_id']."'";
			$result_query = $obj_db->query($sql);
			$result = array(
				'status' => 'success',
				'sql'	=> $sql
			);
		...
		if($_GET['type']=="confirmJob"){
			...
			$sql_update = "UPDATE connotes SET `status`='comfirm',`updated_at`='".$date_now."' WHERE bookings='".$booking_id."'";
			$result_update = $obj_db->query($sql_update);
			echo json_encode($result);
		}
		...
		if($_GET['type']=="getSend"){
			...
			$result_job = $obj_db->getdata($sql,"jobs.msg_key='".$msg_key."' 
				...
				OR (jobs.`status` ='complete' 
					...
					AND jobs.msg_key='".$msg_key."') ORDER BY jobs.`status` ASC","*,jobs.id AS id_jobs,jobs.key AS `key`,jobs.`status` as status ");
		...
		if($_GET['type']=="updateSend"){
			...
			$sql_update = '';
			if($status=="pending"){
				$sql_update = "UPDATE jobs SET `status`='".$status."',updated_at='".$date_now."' WHERE jobs.id='".$_GET['id']."'";
			}else{
				$sql_update = "UPDATE jobs SET `status`='".$status."',updated_at='".$date_now."' WHERE jobs.id='".$_GET['id']."'";
			}
			$result_update = $obj_db->query($sql_update);
