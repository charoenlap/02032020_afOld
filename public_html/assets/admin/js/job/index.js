...
function genViewDetail(job)
{
	...
	console.log(job);
	html += '<form action="#" method="POST" id="form-save-detail">';
		html += '<div class="row">';
			html += '<div class="col-sm-6">';
			html += '<h5 class="mw-left">';
			html += 'สถานะ : <span class="blue-800">'+job.status_label+'</span>';
			html += '</h5>';
			html += '<input type="hidden" name="job_id" id="job_id" value="'+job.id+'">';
			html += '<div class="row">';
				html += '<div class="col-sm-6">';
					html += '<h5 class="mw-left">';
					html += 'ชื่อผู้เซ็นรับของ :';
					html += '</h5>';
				html += '</div>';
				html += '<div class="col-sm-6">';
					html += ' <input type="text" class="form-control txt_job_name" id="txt_job_name" job_id="'+job.id+'" value="'+((job.notes) ? job.notes : '')+'">';
					html += '<p id="result_update_receiver_name"></p>';
				html += '</div>';
			html += '</div>';
			html += '<div class="row">';
				html += '<div class="col-sm-12">';
					html += '<input type="submit" class="btn btn-primary" value="บันทึก">';
				html += '</div>';
			html += '</div>';
			html += '<h5 class="mw-left">';
